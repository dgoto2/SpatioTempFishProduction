;########################################################################################################
PRO WalleyeGrowLakeYear
; parameterize growth models via optimization for each stock (lake or population)
PRINT, 'a program to compute individual growth of walleye stocks in Wisconsin, USA'
PRINT, 'Created: September 1, 2013'
...
; All lakes with reconstructed age structure
file = FILEPATH('walleye_lake_year_Length_Age_reconst_IDL.csv', Root_dir = 'C:' $
  , SUBDIR = 'Users\Daisuke\Desktop\Walleye_production_project\Data')
...
;N = 107866L; all lakes
N = 273872L; all lakes from reconstructed age structure
...
;The mpfit routines allow for some additional control over the fitting. For example, what if you had some information 
;that the amplitude of the gaussian had to be greater than zero? What if you knew that the centroid was precisely 10.2? 
;You want to provide the fitting routine with this information so that it doesn't go off on the wrong path.

;Control information is passed through the structure variable parinfo, which is easier to learn by example rather 
;than by explanation (there is detailed documentation in the mpfitfun.pro file). Here's an example of parinfo for 
;the situation I just described.
;parinfo = replicate({value:0.0, fixed:0, limited:[0,0], limits:[0.0,0.0]}, 3)

;This statement initializes the control array to its defaults; i.e., all parameters vary freely. The last number is 
;the number of parameters in your model; for the gaussian, 3.
...
year = indgen(max(SurveyYear2)-min(SurveyYear2)+1L)+min(SurveyYear2)    ; an index for year
...
;  MAXITER = 100000
;  num_param = 3.
;  parms = mpfitfun('vonBertalanffy', Age_Gro, Length_Gro, dy, startparms, DOF=dof, BESTNORM=bestnorm, $
...
;    ;pi(0).limited(1) = 0; 0=vary; 1=constant
;    ;pi(0).limits(1) = 2000.
;    parms = mpfitfun('vonBertalanffy', Age_Gro[MALE], Length_Gro[MALE], DOF=dof, dy, startparms, $
;      BESTNORM=bestnorm, MAXITER=MAXITER, perror=dparms, yfit=yfit, PARINFO=pi)
...
;    ;pi(0).limited(1) = 0; 0=vary; 1=constant
;    ;pi(0).limits(1) = 2000.
;    parms = mpfitfun('vonBertalanffy', Age_Gro[FEMALE], Length_Gro[FEMALE], dy, startparms, DOF=dof, $
;      BESTNORM=bestnorm, MAXITER=MAXITER, perror=dparms, yfit=yfit, PARINFO=pi)
